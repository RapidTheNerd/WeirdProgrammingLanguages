; This is a web-server in Lisp. Lisp has too many parentheses and it makes me want to :(((
   ...
   (labels ((f (lst)
               (when lst
                 (case (car lst)
                     ...
                     (otherwise (cons (car lst) (f (cdr lst)))))))) ; THIS IS TOO MANY
         ...
         (h (let ((i (position #\: s)))
               (when i 
                     (cons (intern (string-upcase (subseq s 0 i)))
                           (subseq s (+ i 2))))))) ; I don')t kn)o)w why) y)ou do th)is
     (when h
